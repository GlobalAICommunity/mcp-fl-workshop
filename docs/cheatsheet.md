...
then returns parsed calls for `qwen3.5-4b-generic-cpu`; the host handles `final_answer`
...
The event image uses cached CPU model `qwen3.5-4b-generic-cpu`. Attendee code must not call a
